...
svg
  ...
  // Calculate the range of edited lines (min and max)
const [minLines, maxLines] = d3.extent(commits, (d) => d.totalLines);

  // Create a scale for radius based on the number of edited lines
  const rScale = d3
...
function brushed(event) {
  brushSelection = event.selection;  // Update brush selection
  updateSelection();  // Update the visual state of the dots
...
function updateSelection() {
  // Update the 'selected' class on the dots based on the brush selection
...
function isCommitSelected(commit) {
  ...
  const x = xScale(commit.datetime);  // Use xScale for commit's datetime
  const y = yScale(commit.hourFrac);  // Use yScale for commit's hourFrac
